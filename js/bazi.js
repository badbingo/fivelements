document.addEventListener('DOMContentLoaded', function() {
    ...
    calculateBtn.innerHTML = '<i class="fas fa-shopping-cart"></i> 点击付费';
    ...
    const paymentConfig = {
        ...
        amount: '50.00', // 支付金额50元
        paymentType: 'alipay' // 支付类型，alipay或wxpay
    };
    ...
    function checkPaymentStatus() {
        const status = localStorage.getItem('baziPaymentStatus');
        if (status) {
            try {
                const parsed = JSON.parse(status);
                paymentStatus.paid = parsed.paid || false;
                paymentStatus.currentUser = parsed.currentUser || null;
    ...
    function savePaymentStatus() {
        localStorage.setItem('baziPaymentStatus', JSON.stringify(paymentStatus));
    }
    ...
    function generateOrderNo() {
        return 'bazi_' + Date.now() + '_' + Math.floor(Math.random() * 1000);
    }
    ...
    function initiatePayment() {
        ...
        const params = {
            ...
            notify_url: window.location.href, // 实际使用中应该替换为你的回调URL
            return_url: window.location.href, // 支付成功后跳转回当前页面
            name: '八字测算服务',
            money: paymentConfig.amount,
            param: JSON.stringify({
                userId: 'user_' + Date.now(),
                service: 'bazi_calculation'
            }),
            sign_type: 'MD5'
        };
        ...
        form.method = 'post';
        form.action = paymentConfig.apiUrl;
    ...
    // 检查URL中的支付结果参数
    function checkUrlForPaymentResult() {
    ...
    calculateBtn.addEventListener('click', function(e) {
        ...
        // 如果已支付，执行测算逻辑
        if (paymentStatus.paid) {
            originalCalculateBazi(e); // 调用原来的测算函数
            return;
        }
    ...
    recalculateBtn.addEventListener('click', function() {
        ...
        paymentStatus.currentUser = null;
        savePaymentStatus();
        ...
        // 调用原来的重新测算逻辑
        originalRecalculateBazi();
    });
    
    // 保存原来的重新测算函数
    const originalRecalculateBazi = function() {
...
`;
    ...
    // 计算八字
    async function originalCalculateBazi(e) {
        e.preventDefault();
        resetAllContent();
