document.addEventListener('DOMContentLoaded', function() {
    ...
    const baziCache = {
        ...
        set: function(key, value) {
            ...
            if (Object.keys(this.storage).length >= this.maxSize) {
                const oldestKey = Object.keys(this.storage)[0];
        ...
        cleanup: function() {
            ...
            Object.keys(this.storage).forEach(key => {
                if (now > this.storage[key].expiry) {
                    delete this.storage[key];
                }
            });
        },
    ...
    const requestBatch = {
        ...
        processBatch: async function() {
            ...
            try {
                const groupedRequests = this.groupRequests(this.queue);
                ...
                results.flat().forEach(({request, response}) => {
                    if (request.callback) {
        ...
        groupRequests: function(requests) {
            return requests.reduce((groups, request) => {
                const key = request.section || 'default';
                if (!groups[key]) {
                    groups[key] = [];
                }
        ...
        processRequestGroup: async function(group) {
            if (group.length === 1) {
            ...
            const mergedPrompt = this.mergePrompts(group);
            
            try {
                const response = await this.executeBatchRequest(mergedPrompt);
                return this.splitResponse(response, group);
            } catch (error) {
        ...
        mergePrompts: function(requests) {
            return requests.map(r => r.prompt).join('\n\n---\n\n');
        },
        
        splitResponse: function(response, requests) {
        ...
        executeSingleRequest: async function(request) {
            try {
        ...
        executeBatchRequest: async function(prompt) {
    ...
    const apiErrorHandler = {
        ...
        handle: async function(apiCall, args, retries = 0) {
            try {
                return await apiCall(...args);
            } catch (error) {
                console.error(`API调用失败 (尝试 ${retries + 1}/${this.maxRetries + 1}):`, error);
                
                if (this.shouldRetry(error) && retries < this.maxRetries) {
                    await this.delay(this.retryDelay * (retries + 1));
                    return this.handle(apiCall, args, retries + 1);
                }
        ...
        transformError: function(error) {
            ...
            if (error.message.match(/4\d{2}/)) {
                return new Error('请求参数错误，请稍后重试');
            }
            if (error.message.match(/5\d{2}/)) {
                return new Error('服务器繁忙，请稍后再试');
            }
            return error;
        },
    ...
    async function calculateBazi(e) {
        ...
        try {
            ...
            const baziInfo = await apiErrorHandler.handle(
                getBaziAnalysis, 
                ['basic', birthData]
            );
            ...
            alert(error.message || '量子测算失败，请稍后重试');
    ...
    function initElementChart(baziInfo) {
        if (!elementChartDescription) {
            ...
            elementChartDescription = document.createElement('div');
        }
    ...
    // 获取所有天干地支元素 - 确保这些元素有正确的class
    const stemsAndBranches = document.querySelectorAll('.bazi-pillar span, .hidden-stems span');
    ...
    function handleTenGodClick() {
        const char = this.textContent;
        const dayStem = document.getElementById('day-stem').textContent; // 获取日干
        ...
        // 显示十神信息
        if (tenGod) {
            showTenGodTooltip(this, tenGod);
        }
    }
}

function showTenGodTooltip(element, tenGod) {
    // 移除现有的tooltip
    ...
    async function getBaziAnalysis(section, data) {
        ...
        return new Promise((resolve, reject) => {
            const prompt = generateAnalysisPrompt(section, data);
    ...
    function generateAnalysisPrompt(section, data) {
        const localResult = calculateBaziLocally(data);
...
4. 起运时间计算方法
    * 起运岁数 =（出生到下一个节气或上一个节气的天数）÷ 3

当前日期：${currentDateStr}
根据以下八字信息分析${section}相关内容：
姓名：${data.name || '未提供'}
出生日期：${data.date}
出生时间：${data.time} 
...
2 流年财运分析(1-5星)
3 大运财运分析(1-5星)`,
            'elements': `分析八字五行强弱，燥湿和流通情况：
1 五行强弱分析
2 五行燥湿分析
3 五行流通分析
4 调候建议`,
            'personality': `分析命主脾气性格：
1 外在性格分析
2 内在性格分析
3 特殊性格分析`,
            'children': `分析子女情况：
...
3 养生建议
4 流年健康分析`,
            'annual-fortune': `详细分析当前流年运势：
1 流年事业吉凶分析(1-5星)
2 流年婚姻吉凶分析(1-5星)
3 流年重大事件吉凶分析(1-5星)`,
            'daily-fortune': `详细分析每日运势：
1 每日吉凶时辰
2 每日宜忌事项
3 每日冲煞方位`,
            'milestones': `分析一生重要节点和重大灾祸：
1 一生重要事件分析
2 一生重大灾祸分析
3 如何趋吉避凶`,
            'decade-fortune': `分析十年大运走势：
1 全部大运财运吉凶分析(1-5星)
2 大运重大事件吉凶分析(1-5星)`,
            'monthly-fortune': `详细分析今年每月运势：
1 事业吉凶分析(1-5星)
2 婚姻吉凶分析(1-5星)
3 重大事件吉凶分析(1-5星)`
        };

        prompt += sectionPrompts[section] || `请分析${section}相关内容`;
...
7.实现专业排版效果`;
    ...
    async function getBaziAnswer(question) {
        ...
        try {
            const prompt = generateQAPrompt(question);
            const response = await apiErrorHandler.handle(
                executeQARequest,
                [prompt]
            );
...
用户问题：${question}`;
    ...
    async function executeQARequest(prompt) {
        const apiUrl = 'https://api.deepseek.com/v1/chat/completions';
        const apiKey = 'sk-b2950087a9d5427392762814114b22a9';
