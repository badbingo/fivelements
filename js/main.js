...
1. 年柱确定
    * 严格以立春为分界（非农历春节）
    * 示例：2000年2月4日20:32立春，此时刻前为「己卯年」，之后为「庚辰年」
2. 月柱确定
    * 节气分月规则（精确到分钟）：
      月份-节气起点-五虎遁口诀
      正月-立春-甲己之年丙作首
      二月-惊蛰-乙庚之岁戊为头
      注意：节气交接日需对比出生具体时间（如清明4月4日16:28，此时间前算二月）
3. 日柱确定
    * 公历日期→儒略日数→干支计算公式（需兼容1582年10月15日前儒略历）
    * 时区处理：UTC+8北京时间，但新疆等地区建议用真太阳时校正
4. 时柱确定
    * 早子时（0:00-0:59）：按当日干支，时干用五鼠遁（如甲日早子时为甲子）
    * 晚子时（23:00-23:59）：时柱干支算次日，时干仍用当日五鼠遁（如甲日23:30为丙子时）
5. 从强格判定
    * 量化标准：
    * 印比总分数 ≥ 80分（天干1分，地支主气2分，中气1分）
    * 克泄耗十神（财官食伤）均无根或受制
6. 从弱格判定
    * 补充细则：若日主唯一根气被合化（如卯戌合火），按从弱处理
7. 特殊格局
    * 专旺格需满足：
        * 地支三会/三合本局（如曲直格需亥卯未或寅卯辰）
        * 天干无破格十神（如润下格见戊土克水则破）
8. 排大运方向
    * 阳年男性顺排 / 阴年女性顺排 → 找下一个换月节气
    * 阴年男性逆排 / 阳年女性逆排 → 找上一个换月节气
9. 起运时间计算（易错点）
    1. 顺排示例
        * 阳男1973年2月2日案例中，您明确：未过立春 → 年柱壬子，顺排到立春（而非惊蛰），精确到小时计算（1天14小时4分 → 6个月10天）
    2. 逆排陷阱
        * 阴男1995年8月8日案例：逆排需找上一个换月节气（小暑，非立夏），避免误用“上一个节气”字面意思
    3. 其他易错场景
        * 节气交接日出生需核对精确时间（如立春8:00出生，7:59 vs 8:01截然不同）
        * 跨年逆排提示“找上年大雪”是重要补充（多数人忽略年柱切换）
