...
1. 年柱确定：以立春为分界，不是农历春节，立春前为上年干支，立春后为当年干支
2. 月柱确定：严格按节气划分月份：
     正月：立春开始    二月：惊蛰开始
     三月：清明开始    四月：立夏开始
     五月：芒种开始    六月：小暑开始
     ...
     冬月：大雪开始    腊月：小寒开始
     使用五虎遁年上起月法确定月干
3. 日柱确定：按公历日期计算，使用精确公式计算，考虑时区转换（北京时间东八区）
4. 时柱确定：23:00-00:59为子时（早子时），00:00-00:59为晚子时（需特殊处理）
   使用五鼠遁日上起时法确定时干
5. 从强格判定条件：印比力量占80%以上，全局无有力克泄耗（官杀、食伤、财星），必须满足调候用神条件
6. 从弱格判定条件：印比力量不足20%，全局无有力生扶（印星、比劫），必须满足调候用神条件
7. 特殊格局：专旺格（曲直、炎上、稼穑、从革、润下），化气格，两神成象格
8. 排大运基本规则：年干为阳的男性、年干为阴的女性：顺排，年干为阴的男性、年干为阳的女性：逆排，阳年：甲、丙、戊、庚、壬，阴年：乙、丁、己、辛、癸
9. 起运时间计算：顺排：计算从出生时刻到下一个换月节气的时间，逆排：计算从出生时刻到上一个换月节气的时间，精确到分钟，三天折合一岁

1. 年柱：以立春为界（非春节），立春前属上年干支
2. 月柱：按节气划分月份（如正月立春起，二月惊蛰起）
3. 日柱：公历日期计算（需精确公式）
4. 时柱：23-0点为子时（早子时按次日日干）

【格局判断】
5. 从强格：日主得月令且印比≥80%全局力量
6. 从弱格：日主失月令且印比≤20%全局力量

【大运规则】
7. 阳年男/阴年女顺排，阴年男/阳年女逆排
   - 阳年：甲丙戊庚壬
   - 顺排：出生→下一节气时间计算
   - 逆排：出生→上一节气时间计算
