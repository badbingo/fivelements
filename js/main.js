document.addEventListener('DOMContentLoaded', function() {
    ...
    const baziCache = {
        ...
        clearExpired: function() {
            ...
            for (const key in this.data) {
                if (now - this.data[key].timestamp > this.data[key].ttl) {
    ...
    // 每小时清理一次过期缓存
    setInterval(() => baziCache.clearExpired(), 3600000);
    ...
    const apiConfig = {
        ...
        apiKey: 'sk-b2950087a9d5427392762814114b22a9',
        model: "deepseek-chat",
        timeout: 10000,
        retryCount: 2,
        retryDelay: 1000
    ...
    // ==================== API相关函数 ====================
    async function makeApiRequest(prompt, section) {
        const cacheKey = `${generateBaziHashKey(birthData)}:${section}`;
        ...
        // 检查兜底规则
        const fallbackKey = `${currentPillars.year}${currentPillars.month}${currentPillars.day}${currentPillars.hour}`;
        if (fallbackRules[fallbackKey] && section === 'basic') {
            return fallbackRules[fallbackKey];
        }
        ...
        for (let attempt = 0; attempt <= apiConfig.retryCount; attempt++) {
            try {
                ...
                lastError = error;
                console.error(`API请求尝试 ${attempt + 1} 失败:`, error);
        ...
        // 所有尝试都失败后，返回本地计算结果
        console.warn('API请求失败，使用本地计算结果', lastError);
                ...
                fn: async () => {
                    try {
                        const responses = await Promise.all(
                            requests.map(req => makeApiRequest(req.prompt, req.section))
                        );
                        return responses;
                    } catch (error) {
                        throw error;
...
7.实现专业排版效果`;
        ...
        try {
            const response = await makeApiRequest(prompt, section);
            return response;
        } catch (error) {
...
用户问题：${question}`;
        ...
        try {
            const response = await makeApiRequest(prompt, 'qa');
            return response;
        } catch (error) {
...
function hideTenGodsTooltip() {
    ...
    function preloadCommonSections() {
        const commonSections = ['strength', 'career', 'wealth'];
        const requests = commonSections.map(section => ({
            section,
            prompt: generatePromptForSection(section)
        }));
        
        batchApiRequests(requests)
            .then(responses => {
                responses.forEach((response, index) => {
                    const section = commonSections[index];
                    baziCache.set(`${generateBaziHashKey(birthData)}:${section}`, response);
                });
            })
            .catch(error => {
    ...
    function generatePromptForSection(section) {
        const currentDateStr = currentDate.getFullYear() + '-' + 
                              (currentDate.getMonth() + 1).toString().padStart(2, '0') + '-' + 
                              currentDate.getDate().toString().padStart(2, '0');
