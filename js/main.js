document.addEventListener('DOMContentLoaded', function() {
    ...
    loadSavedProfiles();
    updateLunarCalendar();
    setInterval(() => cache.clearExpired(), 60000); // 每分钟清理一次过期缓存
    ...
    function validateBaziResult(localResult, apiResult) {
        // 关键字段对比
        const keyFields = ['yearStem', 'yearBranch', 'monthStem', 'monthBranch', 'dayStem', 'dayBranch', 'hourStem', 'hourBranch'];
        let isValid = true;
    ...
    // 从API响应中提取关键字段
    function extractKeyFieldsFromApiResponse(responseText) {
        // 使用正则表达式提取关键字段
        const yearMatch = responseText.match(/年柱\[([^\]]+)\]/);
        const monthMatch = responseText.match(/月柱\[([^\]]+)\]/);
        const dayMatch = responseText.match(/日柱\[([^\]]+)\]/);
        const hourMatch = responseText.match(/时柱\[([^\]]+)\]/);
    ...
    async function getBaziAnalysis(section, data) {
        ...
        // 本地计算基础排盘
        const localBazi = calculateBaziLocally(data);
...
用Markdown格式，段落与段落之间空一行，使用分隔线，标题和重要内容高亮显示，添加视觉引导元素如箭头、进度条等，不要使用任何特殊符号`;
        ...
        try {
            ...
            // 如果是基础排盘部分，进行校验
            if (section === 'basic') {
                const apiBazi = extractKeyFieldsFromApiResponse(content);
                if (apiBazi) {
                    const isValid = validateBaziResult(localBazi, apiBazi);
                    
                    if (!isValid) {
                        // 如果不一致，尝试使用兜底规则
                        const baziKey = `${localBazi.yearStem}${localBazi.yearBranch}${localBazi.monthStem}${localBazi.monthBranch}${localBazi.dayStem}${localBazi.dayBranch}${localBazi.hourStem}${localBazi.hourBranch}`;
                        if (fallbackRules[baziKey]) {
                            console.warn('使用兜底规则库中的排盘数据');
                            return JSON.stringify({
                                ...fallbackRules[baziKey],
                                elements: localBazi.elements,
                                personality: localBazi.personality
                            });
                        }
                        
                        // 如果没有兜底规则，使用本地计算结果
                        console.warn('API排盘不一致，使用本地计算结果');
                        return JSON.stringify(localBazi);
                    }
                }
            }
