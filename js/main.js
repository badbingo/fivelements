document.addEventListener('DOMContentLoaded', function() {
    ...
    const backupRules = {
        ...
        "甲子-丙寅-壬午-丙午": {
            ...
            dayHiddenStems: "丁己",
            hourHiddenStems: "丁己",
            personality: "似参天大树，正直向上，有领导力但略显固执"
        }
    };
    ...
    function generateCacheKey(type, content) {
        const input = `${type}:${birthData.date}:${birthData.time}:${birthData.gender}:${content}`;
        return sha256(input);
    }

    // 简单的SHA256实现（生产环境应使用更完整的实现）
    function sha256(input) {
        // 简化实现，实际应用中应使用完整SHA256算法
        let hash = 0;
    ...
    function validateBaziResult(apiResult, localResult) {
        ...
        for (const field of keyFields) {
            if (apiResult[field] !== localResult[field]) {
        ...
        // 藏干校验
        const hiddenStemFields = ['yearHiddenStems', 'monthHiddenStems', 
                                'dayHiddenStems', 'hourHiddenStems'];
        
        for (const field of hiddenStemFields) {
            const apiStems = apiResult[field] ? apiResult[field].split('').sort().join('') : '';
            const localStems = localResult[field] ? localResult[field].split('').sort().join('') : '';
            
            if (apiStems !== localStems) {
                console.warn(`藏干不一致: ${field} 本地:${localStems} API:${apiStems}`);
                // 使用本地计算结果
                apiResult[field] = localResult[field];
            }
        }
    ...
    // 从API响应中提取关键字段
    function extractKeyFieldsFromAPIResponse(response) {
        // 使用正则表达式提取关键信息
        const yearMatch = response.match(/年柱\[([^\]]+)\]/);
        const monthMatch = response.match(/月柱\[([^\]]+)\]/);
        const dayMatch = response.match(/日柱\[([^\]]+)\]/);
        const hourMatch = response.match(/时柱\[([^\]]+)\]/);
        
        const yearHiddenMatch = response.match(/年支藏干\[([^\]]+)\]/);
        const monthHiddenMatch = response.match(/月支藏干\[([^\]]+)\]/);
        const dayHiddenMatch = response.match(/日支藏干\[([^\]]+)\]/);
        const hourHiddenMatch = response.match(/时支藏干\[([^\]]+)\]/);
        
        const elementsMatch = response.match(/五行能量\[([^\]]+)\]/);
        const personalityMatch = response.match(/命主性格\[([^\]]+)\]/);
        
        return {
            yearStem: yearMatch ? yearMatch[1].charAt(0) : '',
            yearBranch: yearMatch ? yearMatch[1].charAt(1) : '',
            monthStem: monthMatch ? monthMatch[1].charAt(0) : '',
            monthBranch: monthMatch ? monthMatch[1].charAt(1) : '',
            dayStem: dayMatch ? dayMatch[1].charAt(0) : '',
            dayBranch: dayMatch ? dayMatch[1].charAt(1) : '',
            hourStem: hourMatch ? hourMatch[1].charAt(0) : '',
            hourBranch: hourMatch ? hourMatch[1].charAt(1) : '',
            ...
            elements: elementsMatch ? parseElements(elementsMatch[1]) : [0, 0, 0, 0, 0],
            personality: personalityMatch ? personalityMatch[1] : ''
        };
    }

    function parseElements(elementsStr) {
        const matches = elementsStr.match(/(\d+)/g);
    ...
    async function calculateBazi(birthData) {
        ...
        // 生成缓存键
        const cacheKey = generateCacheKey('bazi', 'basic');
        ...
        // 2. 调用API获取详细分析
        try {
            const apiResponse = await getBaziAnalysis('basic', birthData);
            const extracted = extractKeyFieldsFromAPIResponse(apiResponse);
            
            // 3. 合并结果
            const mergedResult = {...localResult, ...extracted, analysis: apiResponse};
            
            // 4. 校验结果
            if (!validateBaziResult(mergedResult, localResult)) {
                console.warn('排盘校验不通过，使用本地计算结果');
                // 使用本地结果，但保留API的分析部分
                return {...localResult, analysis: apiResponse};
            }
            
            // 5. 缓存结果
            baziCache[cacheKey] = mergedResult;
            return mergedResult;
        } catch (error) {
            console.error('API调用失败，使用本地计算结果:', error);
            // 检查兜底规则库
            const pillarsKey = `${localResult.yearStem}${localResult.yearBranch}-${localResult.monthStem}${localResult.monthBranch}-${localResult.dayStem}${localResult.dayBranch}-${localResult.hourStem}${localResult.hourBranch}`;
            
            if (backupRules[pillarsKey]) {
                console.log('使用兜底规则库');
                return {...localResult, ...backupRules[pillarsKey]};
            }
    ...
    function calculateBaziLocally(birthData) {
        ...
        return {
            ...
            gamblingFortune: gamblingFortune
        };
    }
    ...
    async function getBaziAnswer(question) {
        const cacheKey = generateCacheKey('qa', question);
...
用户问题：${question}`;
        ...
        try {
            const response = await fetch(apiUrl, {
                ...
                body: JSON.stringify({
                    ...
                    temperature: 0,
                    seed: generateSeed(question) // 固定seed确保相同问题相同回答
                })
            });
    ...
    async function getBaziAnalysis(section, data) {
        const cacheKey = generateCacheKey(section, 'analysis');
        ...
        try {
            const response = await fetch(apiUrl, {
                ...
                body: JSON.stringify({
                    ...
                    messages: [{ role: "user", content: prompt }],
                    temperature: 0,
                    seed: generateSeed(section) // 固定seed确保相同部分相同回答
                })
            });
            ...
            // 对于基础部分，提取关键字段
            if (section === 'basic') {
                const extracted = extractKeyFieldsFromAPIResponse(content);
                // 合并本地计算结果
                const localResult = calculateBaziLocally(data);
                const finalResult = {...localResult, ...extracted, analysis: content};
                
                // 缓存结果
                baziCache[cacheKey] = finalResult;
                return finalResult;
            }
        ...
        document.querySelectorAll('.load-btn').forEach(button => {
            ...
            button.addEventListener('click', async function(e) {
                ...
                try {
                    ...
                    clearInterval(progressInterval);
                    contentElement.innerHTML = '<p style="color:var(--danger-color)">加载失败，请稍后重试</p>';
                    button.disabled = false;
