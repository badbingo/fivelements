 ...
 `;
   ...
   async getUsername() {
     try {
       const token = localStorage.getItem('token');
       if (!token) {
         this.log('用户未登录，无法获取用户名');
         return null;
       }
       
       const response = await fetch(`${this.config.paymentGateway.apiBase}/api/user/info`, {
         headers: {
           'Authorization': `Bearer ${token}`
         }
       });
       ...
       if (!response.ok) {
         throw new Error(`获取用户名失败: ${response.status}`);
       }
       
       const data = await response.json();
       return data.username || null;
     } catch (error) {
       this.safeLogError('获取用户名失败', error);
       return null;
     }
   }
